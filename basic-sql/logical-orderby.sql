-- Query that returns all rows from 2012, ordered by song title from Z to A.
SELECT *
...
-- Query that shows all rows for which T-Pain was a group member, ordered by rank on the charts, from lowest to highest rank (from 100 to 1).
SELECT *
  FROM tutorial.billboard_top_100_year_end
WHERE group_name ILIKE '%t_pain%'
