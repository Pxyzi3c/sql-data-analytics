...
FROM
  tutorial.crunchbase_acquisitions AS a
  LEFT JOIN tutorial.crunchbase_companies AS c ON a.company_permalink = c.permalink
...
WHERE
  c.state_code IS NOT NULL
GROUP BY
  c.state_code
ORDER BY
  acquired_companies_count DESC
...
FROM
  tutorial.crunchbase_companies AS c
  LEFT JOIN tutorial.crunchbase_investments AS i ON c.permalink = i.company_permalink
WHERE
  c.founded_year + 5 < i.funded_year
GROUP BY
  c.name,
  c.status,
  c.category_code